...
recursive_max = (
    float(config_data["recursive_max"]) if "recursive_max" in config_data else None
)
...
submission_filename = os_listdir("/autograder/submission/")[
    0
]  # we assume a file exists, as a student must submit a file for the program to run
...
exists = (
    ...
    and source_path.split(".")[-1] == file_extension_required
)
...
late_due_time = datetime.fromisoformat(
    submission_metadata["assignment"]["late_due_date"]
)
...
for testcaseidx in range(num_testcases):
    with open(f"./testcases/Input/{testcaseidx + 1}.in", "r") as fin:
        tc_stdin = fin.read()
    r(
        f"stdbuf -oL ./{bin_name} <./testcases/Input/{testcaseidx + 1}.in >test-{testcaseidx + 1}.out"
    )
    ...
    passed = [i for i in student_stdout if i not in b" \n\t\r"] == [
        i for i in tc_stdout if i not in b" \n\t\r"
    ]
    if passed:
...
if recursive_max:
    ...
    probably_recursive = (
        r(f'echo "{tc_stdin}" | ./{bin_name} 2>dev/null; echo $?') == b"139"
    ) and (b"x28" in source_code or b"sp" in source_code)
